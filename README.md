  ...
  <style>
    ...
    * {
    ...
    .fretboard {
      position: relative;
      display: grid;
      grid-template-columns: repeat(12, var(--fret-width));
      grid-template-rows: repeat(6, var(--string-gap));
...
.note.open.selected {
  ...
  color: #111;
    ...
    /* Give the Fill selected / Unfill selected buttons the same highlight shadow */
    #fill-selected, #unfill-selected {
      box-shadow: 0 10px 30px rgba(30,130,255,0.12), 0 0 8px rgba(30,130,255,0.5);
    }
    #fill-selected:hover, #unfill-selected:hover {
...
<div style="margin-top: 30px; background: white; padding: 24px; border-radius: var(--border-radius); box-shadow: var(--shadow-lg);">
  <div style="display: flex; align-items: center; gap: 24px; flex-wrap: wrap;">
    <div>
      ...
      <button id="export-svg">Exporter SVG - not yet</button>
    </div>
    <div class="fill-buttons">
      <button id="fill">Fill</button>
      <button id="fill-grey" class="grey-btn">Fill in grey</button>
      <button id="fill-selected">Fill selected</button>
      <button id="unfill-all">Unfill all</button>
      <button id="unfill-grey" class="grey-btn">Unfill grey</button>
      <button id="unfill-selected">Unfill selected</button>
    </div>
    ...
    <div style="display: flex; align-items: center; gap: 12px;">
      ...
      <input type="text" id="title" value="fretboard" style="padding: 8px 12px; border: 2px solid #e5e7eb; border-radius: 6px; font-size: 15px; font-weight: 600; font-family: inherit; min-width: 200px;">
    </div>
  </div>
</div>

<div style="margin-top: 24px; padding: 20px 24px; background: white; border-radius: var(--border-radius); box-shadow: var(--shadow-md); border-left: 4px solid var(--primary-color);">
  <p style="margin: 0; font-size: 15px; line-height: 1.6; color: #4b5563;">
    <strong style="color: #1f2937;">Instructions:</strong><br>
    • <strong>Click</strong> on a cell to add or remove a note<br>
    • <strong>Shift + Click</strong> for a special note (e.g., root)<br>
    • <strong>Ctrl + Click</strong> to select a note
  </p>
</div>  
...
<script>
  ...
  const OPEN_NOTES = [5, 12, 8, 3, 10, 5]; // e, b, g, d, a, e
  function getNote(string, fret) {
    return ((OPEN_NOTES[string] + fret - 1) % 12) + 1;
  }

  const fretWidth = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--fret-width'));
  const stringGap = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--string-gap'));
  ...
  function xForFret(fret) {
    return (fret - window.currentStartFret + 0.5) * fretWidth;
  }

  function yForString(stringIndex) {
    return stringIndex * stringGap + stringGap / 2;
  }
  ...
  function updateFretboard() {
    ...
    // Set grid
    fretboard.style.gridTemplateColumns = `repeat(${numFrets}, var(--fret-width))`;
    fretboard.style.borderLeft = showOpen ? '6px solid #222' : 'none';
    ...
    if (DOUBLE_MARKER_FRET >= minFret && DOUBLE_MARKER_FRET <= maxFret) {
      const topMarkerY = 2 * stringGap;
      const bottomMarkerY = 4 * stringGap;
    ...
    if (!showOpen) {
      ...
      label.style.left = `${0.5 * fretWidth}px`;
  ...
  (function () {
    ...
    function updateIndicatorFromEvent(e) {
      ...
      let stringIndex = Math.floor(y / stringGap);
      stringIndex = Math.max(0, Math.min(STRINGS - 1, stringIndex));

      let fret;
      const openThreshold = fretWidth * 0.25;
      if (window.currentShowOpen && x < openThreshold) {
        fret = 0;
      } else {
        fret = Math.ceil(x / fretWidth) + window.currentStartFret - 1;
        fret = Math.max(window.currentMinFret, Math.min(window.currentMaxFret, fret));
      }

      const left = fret === 0 ? -3 : xForFret(fret);
  ...
  document.getElementById('fill-grey').addEventListener('click', () => {
  ...
  document.getElementById('unfill-all').addEventListener('click', () => {
    window.filledNotes = [];
    window.greyNotes = [];
    updateFretboard();
  });

  document.getElementById('unfill-grey').addEventListener('click', () => {
    window.greyNotes = [];
    updateFretboard();
  });
  ...
  document.getElementById('fill-selected').addEventListener('click', () => {
    ...
    for (let ss = 0; ss < STRINGS; ss++) {
      for (let ff = 0; ff <= 12; ff++) {
        if (getNote(ss, ff) === targetNote) {
          const alreadyClicked = window.clickedNotes.some(n => n.string === ss && n.fret === ff);
          const alreadyFilled = window.filledNotes.some(n => n.string === ss && n.fret === ff);
          if (!alreadyClicked && !alreadyFilled) {
            window.filledNotes.push({string: ss, fret: ff, type: 'normal', color: sourceColor});
          }
        }
      }
    }
  ...
  document.getElementById('unfill-selected').addEventListener('click', () => {
    if (!window.selectedNoteKey) return;
    const parts = window.selectedNoteKey.split('-');
    const s = parseInt(parts[0].slice(1), 10);
    const f = parseInt(parts[1].slice(1), 10);
...
<script>
  ...
  fretboard.addEventListener("click", (event) => {
    ...
    if (existingIndex !== -1) {
      ...
      const type = event.shiftKey ? "root" : "normal";
      window.clickedNotes.push({string, fret, type, color: window.currentNoteColor});
    }
    updateFretboard();
  });
